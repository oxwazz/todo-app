...
router.post('/', async (req, res) => {
  try {
    ...
    const createTask = await pool.query(
      'INSERT INTO tasks (name, description, date, priority, is_done) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
router
  ...
  .get(async (req, res) => {
    try {
      const { id } = req.params
      const getTask = await pool.query('SELECT * FROM tasks WHERE id=$1', [id])
      const isNulldata = () => {
        getTask.rows == ''
  ...
  .patch(async (req, res) => {
    try {
      const { id } = req.params
      const { description } = req.body
      const updateTask = await pool.query('UPDATE tasks SET description=$1 WHERE id=$2', [
        description,
        id,
      ])
